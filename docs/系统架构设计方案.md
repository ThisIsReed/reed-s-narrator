...
> 文档定位：本文件维护“目标架构与推荐实现方式”，不负责记录逐次开发历史。  
> 当前实现状态与测试结果请优先查看 [`changelog.md`](./changelog.md)，当前缺口与后续顺序请查看 [`系统架构工作包拆分.md`](./系统架构工作包拆分.md)。
...
## 10. 2026-03-11 架构审查补充：后续修复重点

### 10.1 当前实现与目标架构的差距

基于当前仓库实现状态，本方案中有三类能力仍未完全成为“世界按 tick 推进”的正式内核：

1. **通用规则引擎尚未接入主循环**
   - 当前只有物候规则链进入 `run_tick()`；
   - `core/rule_engine.py` 已实现稳定排序、匹配与审计，但仍未成为统一世界结算器。

2. **中断机制尚未进入运行链路**
   - `InterruptManager` 已存在，但 `NarratorController` 尚未显式执行“中断扫描 / 中断结算”阶段；
   - 因此长程任务仍缺少“被外部事件打断”的正式能力。

3. **PASSIVE/DORMANT 持续推进语义不足**
   - 当前 PASSIVE 更新更接近扩展点，而不是正式执行层；
   - 角色模型中的 `status_effects`、`long_action` 还未被统一执行器稳定消费。

### 10.2 需要补入主循环的正式阶段

为使“规则 / 中断 / 被动执行”成为一等公民，后续主循环建议演化为：
...
补充说明：
- `phenology` 继续保留为专用规则子系统，负责季节/气候等特色领域逻辑；
- `passive_execution` 负责角色长期行为推进、持续状态结算和低成本被动变化；
- `world_rules` 作为统一规则出口，负责归并非物候类确定性状态变更；
- `interrupt_scan` 负责把“外界变化 -> 长程任务打断 -> 状态回写”纳入正式阶段链。

### 10.3 后续修复工作包

为避免与已完成工作包混淆，新增三项阶段 A 补强工作包：

1. **WP-14 通用规则引擎接线与统一结算阶段**
   - 把 `RuleEngine` 接入 `NarratorController`；
   - 为 tick 结算建立统一规则上下文；
   - 将非物候类确定性变更收敛到统一规则内核。

2. **WP-15 中断链路与长程任务打断闭环**
   - 把 `InterruptManager` 接入正式主循环；
   - 让 `long_action` 成为可推进、可中断、可恢复的正式运行态；
   - 将中断结果纳入 replay/audit。

3. **WP-16 被动执行层与持续状态推进语义**
   - 建立正式 `PassiveExecutor` 或等价执行组件；
   - 为 `status_effects`、长期行为、被动资源变化提供统一执行协议；
   - 让 PASSIVE/DORMANT 角色在低成本前提下真正参与世界推进。

### 10.4 新增验收要求

后续阶段 A 收口时，除既有验收指标外，还应新增：

1. 至少一条集成测试显式断言 `world_rules` 阶段已进入主循环。
2. 至少一条集成测试覆盖“长程任务 -> 中断触发 -> 状态变更 -> checkpoint/replay 一致”。
3. 至少一条长程测试断言 PASSIVE 角色在无 ACTIVE 行动时仍能推动世界状态变化。
4. 通用规则、中断信号、被动执行结果都必须进入 tick audit 或等价可回放审计面。
